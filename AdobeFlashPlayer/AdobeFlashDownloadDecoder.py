...
class AdobeFlashDownloadDecoder(Processor):
    ...
    input_variables = {
        "encoded_path": {
            "required": True,
            "description": (
                "Path to a downloaded, encoded DMG from Adobe's " "auto-update url."
            ),
        }
    }
    output_variables = {
        "pathname": {
            "description": (
                "Path to the decoded DMG file. Note that this is "
    ...
    def main(self):
        ...
        proc = subprocess.Popen(sec_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
